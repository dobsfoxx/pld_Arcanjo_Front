...
export const mapApiSectionToState = (section: PldSection): Section => {
    ...
    .map((q) => {
      ...
      return {
        ...
        test: {
          ...
          requisicao: [],
          requisicaoRef: q.requisicaoRef || findAtt('TESTE_REQUISICAO')?.referenceText || '',
          resposta: [],
          respostaRef: q.respostaTesteRef || findAtt('TESTE_RESPOSTA')?.referenceText || '',
          amostra: [],
          amostraRef: q.amostraRef || findAtt('TESTE_AMOSTRA')?.referenceText || '',
          evidencias: [],
          evidenciasRef: q.evidenciasRef || findAtt('TESTE_EVIDENCIAS')?.referenceText || '',
          actionPlan: {
  ...
  return {
    id: section.id,
    item: section.item === 'Outro (1)' || section.item === 'Outro (2)' ? 'Outro' : section.item,
