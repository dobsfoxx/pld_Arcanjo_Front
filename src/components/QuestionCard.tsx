...
export const QuestionCard: React.FC<{
  ...
  const selectedEvidenceFiles = useMemo(() => {
    ...
    if (question.respostaArquivo) items.push({ label: 'Resposta', file: question.respostaArquivo });
    if (question.deficienciaArquivo) items.push({ label: 'Deficiência', file: question.deficienciaArquivo });
    if (question.test.requisicao) items.push({ label: 'Requisição', file: question.test.requisicao });
    if (question.test.resposta) items.push({ label: 'Resposta (teste)', file: question.test.resposta });
    if (question.test.amostra) items.push({ label: 'Amostra', file: question.test.amostra });
    if (question.test.evidencias) items.push({ label: 'Evidências', file: question.test.evidencias });
    return items;
  }, [
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <FileUpload
                          label="REQUISIÇÃO"
                          file={question.test.requisicao}
                          onFileSelect={(f) => updateTest({ requisicao: f })}
                          onRemove={() => updateTest({ requisicao: null })}
                          disabled={!canEdit}
                        />
                        <input
                          maxLength={200}
                      ...
                      <div className="space-y-2">
                        <FileUpload
                          label="RESPOSTA"
                          file={question.test.resposta}
                          onFileSelect={(f) => updateTest({ resposta: f })}
                          onRemove={() => updateTest({ resposta: null })}
                          disabled={!canEdit}
                        />
                        <input
                          maxLength={200}
                      ...
                      <div className="space-y-2">
                        <FileUpload
                          label="AMOSTRA"
                          file={question.test.amostra}
                          onFileSelect={(f) => updateTest({ amostra: f })}
                          onRemove={() => updateTest({ amostra: null })}
                          disabled={!canEdit}
                        />
                        <input
                          maxLength={200}
                      ...
                      <div className="space-y-2">
                        <FileUpload
                          label="EVIDÊNCIAS"
                          file={question.test.evidencias}
                          onFileSelect={(f) => updateTest({ evidencias: f })}
                          onRemove={() => updateTest({ evidencias: null })}
                          disabled={!canEdit}
                        />
                        <input
                          maxLength={200}
